 ...
 */
/**
 * Tries to extract an Instagram reel/clip URL from various item shapes.
 * Instagram sends reels as reel_share, clip, xma_clip, felix_share, etc.
 */
function extractReelLink(item: any): string | null {
	// reel_share → media.code
	const reelMedia = item.reel_share?.media ?? item.clip?.clip ?? item.felix_share_media;
	if (reelMedia?.code) {
		return `https://www.instagram.com/reel/${reelMedia.code}/`;
	}

	// xma_clip / xma_media_share — often has a target_url or preview_url
	for (const key of ['xma_clip', 'xma_media_share', 'xma_reel_share']) {
		const xma = item[key];
		if (!xma) continue;

		// Can be an array or single object
		const entries = Array.isArray(xma) ? xma : [xma];
		for (const entry of entries) {
			if (entry.target_url) return entry.target_url;
			if (entry.preview_url) return entry.preview_url;
			if (entry.header_title_text?.includes('instagram.com'))
				return entry.header_title_text;
		}
	}

	// media_share with code (shared posts that are actually reels)
	const mediaShare = item.media_share ?? item.direct_media_share?.media;
	if (mediaShare?.code && mediaShare?.media_type === 2) {
		return `https://www.instagram.com/reel/${mediaShare.code}/`;
	}
...
export function parseMessageItem(
	...
	switch (item.item_type) {
		...
		case MessageSyncMessageTypes.VoiceMedia: {
			const voiceMedia = (item as any).voice_media;
			const audioSrc = voiceMedia?.media?.audio?.audio_src;
			if (audioSrc) {
				return {
					...baseMessage,
					itemType: 'voice' as const,
					voiceUrl: audioSrc,
					voiceDuration: voiceMedia?.media?.audio?.duration ?? 0,
				};
			}
		...
		case MessageSyncMessageTypes.ReelShare: {
			const reelShareLink = extractReelLink(item);
			if (reelShareLink) {
				return {
					...baseMessage,
					itemType: 'link',
					link: {text: 'Reel', url: reelShareLink},
				};
			}
		...
		default: {
			// Reels / clips — extract the link when possible
			const reelLink = extractReelLink(item);
			if (reelLink) {
				return {
					...baseMessage,
					itemType: 'link',
					link: {text: 'Reel', url: reelLink},
				};
			}
